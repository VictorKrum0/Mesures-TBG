...
class OxfordInstruments_IPS120(VisaInstrument):
    ...
    def __init__(self, name, address, use_gpib=False, number=2, **kwargs):
        ...
        self.add_parameter('persistent_mode',
                           get_cmd=self._get_persistent_mode)
    ...
    def _set_B(self, field_value):
        """
        1. Switch on the heater
        2. Run to field (change setpoint to B and go to B)
        3. Switch off the heater
        
        Args:
            field_value (float): Magnetic field value to go in Tesla
        """
        
        # We set the sweep rate as high as possible
        if field_value < 10:
            self.sweeprate_field(0.4)
        elif field_value < 12:
            self.sweeprate_field(0.2)
        else:
            self.sweeprate_field(0.1)
        
        #self.heater_on()
        self.leave_persistent_mode()
        
        self.run_to_field_wait(field_value)
    ...
    def _get_persistent_mode(self):
        field = self.field()
        persistent_field = self.persistent_field()
        return field != persistent_field
    ...
    def _set_switch_heater(self, mode):
        ...
        if mode in [0, 1]:
            ...
            self.local()
            if mode == 0:
                print("Switch off heater... (wait 40s)")
                sleep(40)
            elif mode == 1:
                print("Switch on heater...  (wait 20s)")
                sleep(20)
        else:
            print('Invalid mode inserted.')
        sleep(0.1)
